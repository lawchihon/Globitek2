<?php require_once('../../../private/initialize.php'); ?>

<?php $page_title = 'Staff: States'; ?>
<?php include(SHARED_PATH . '/header.php'); ?>

<div id="main-content">
  <a href="../index.php" class="btn btn-primary">Back to Menu</a><br />

  <h1>States</h1>

  <a href="./new.php" class="btn btn-success">Add a State</a><br />
  <br />

  <?php
    $state_result = find_all_states();

    echo "<table id=\"states\" class=\"table\">";
    echo "<tr class=\"active\">";
    echo "<th>Name</th>";
    echo "<th>Code</th>";
    echo "<th></th>";
    echo "<th></th>";
    echo "</tr>";
    while($state = db_fetch_assoc($state_result)) {
      echo "<tr>";
      echo "<td>" . h($state['name']) . "</td>";
      echo "<td>" . h($state['code']) . "</td>";
      echo "<td>";
      echo "<a href=\"show.php?id=" . u($state['id']) . "\" class=\"btn btn-info\">Show</a>";
      echo "</td>";
      echo "<td>";
      echo "<a href=\"edit.php?id=" . u($state['id']) . "\" class=\"btn btn-success\">Edit</a>";
      echo "</td>";
      echo "</tr>";
    } // end while $states
    db_free_result($state_result);
    echo "</table>"; // #states
  ?>

</div>

<?php include(SHARED_PATH . '/footer.php'); ?>
